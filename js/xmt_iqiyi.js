...
(function (XMT_IQIYI) {
    ...
    function CheckLogin() {
        ...
        var signStr = "user_id=" +
            ...
            "&key=" +
            data.key;
        var sign = XMT_IQIYI.hex_md5(signStr).toLowerCase();
        return sign !== data.sign;
    }
    XMT_IQIYI.CheckLogin = CheckLogin;
    function GameLoaded() {
        SdkPost({
            type: "dataCount",
            msg: "server"
    ...
    XMT_IQIYI.GameLoaded = GameLoaded;
    function AddSign(data) {
        var signObj = Object.assign({
            sign_key: XMT_IQIYI.Config.SignKey
        }, data);
        var dataArr = Object.keys(signObj).sort();
        var signStr = "";
        dataArr.map(function (item, key) {
            signStr += key === 0 ? "" : "&";
            signStr += item + "=" + signObj[item];
        });
        var newData = Object.assign({
            sign: XMT_IQIYI.hex_md5(signStr)
        }, data);
        return newData;
    }
    function GetUserInfoJsonp(data) {
        return new Promise(function (resolve, reject) {
            var callbackData = Object.assign({
                callback: "jsonpCallback"
            }, data);
            var finalData = AddSign(callbackData);
            var url = IQIYI_BASE_API + "/minigame/userInfo?source=" + finalData.source + "&uid=" + finalData.uid + "&game_id=" + finalData.game_id + "&time=" + finalData.time + "&callback=" + finalData.callback + "&sign=" + finalData.sign;
    ...
    XMT_IQIYI.EventCallbacks = {
        ...
        onRewardVideoCached: onRewardVideoCached,
        onRewardADShow: onRewardADShow,
        onRewardADExpose: onRewardADExpose,
        onRewardADReward: onRewardADReward,
        onRewardADClick: onRewardADClick,
        onRewardVideoComplete: onRewardVideoComplete,
        onRewardADClose: onRewardADClose,
        onRewardADError: onRewardADError,
        onRewardADhasExpired: onRewardADhasExpired,
        onRewardADhasShown: onRewardADhasShown,
        onRewardADneedInit: onRewardADneedInit,
        onBannerADReceiv: onBannerADReceiv,
        onNoBannerAD: onNoBannerAD,
        onBannerADExposure: onBannerADExposure,
    ...
    function Init() {
        var data = {
            source: "4",
            uid: XMT_IQIYI.QueryData.user_id,
            game_id: XMT_IQIYI.Config.GameID,
...
//# sourceMappingURL=xmt_iqiyi.js.map